...
#include "BricktronicsEv3Button.h"

BricktronicsEv3Button::BricktronicsEv3Button(uint8_t inputPin):
    _inputPin(inputPin),
    _pinMode(&::pinMode),
...
BricktronicsEv3Button::BricktronicsEv3Button(const BricktronicsSensorSettings &settings):
    _inputPin(settings.DA),
...
void BricktronicsEv3Button::begin(void)
{
   // _pinMode(_inputPin, INPUT_PULLUP);
}

bool BricktronicsEv3Button::isPressed(void)
{
    return (analogRead(_inputPin)>=1023);
}

bool BricktronicsEv3Button::isReleased(void)
{
    return (analogRead(_inputPin)<=1022);
}
