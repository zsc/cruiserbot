...
**/
EV3UARTSensor::EV3UARTSensor(const BricktronicsSensorSettings &settings)
{
	uint8_t rx_pin, tx_pin;
	tx_pin = settings.DA;
	rx_pin = settings.DB;
	ss = new SoftwareSerial(rx_pin, tx_pin);
	status = RESET;
	speed = 2400;
	mode = -1;
	num_samples = 1;
			...
			if ((cmd & CMD_MASK) == CMD_DATA)
			{
				...
				if ((this->type == TYPE_COLOR && mode == 4) || checksum == sum)
				{
					...
					for(int i=0; i<num_samples; i++)
					{
						...
						{
						case 0:
							this->value[i] = (float) bb[i];
							break;
						case 1:
							this->value[i] = (float) get_int(bb,i*2);
							break;
						case 2:
							this->value[i] = (float) get_long(bb,i*4);
							break;
						case 3:
							this->value[i] = get_float(bb,i*4);
							break;
						}
#ifdef DEBUG
