...
class Model:
    ...
    def __init__(self, res18_npy_path):
        assert os.path.exists(res18_npy_path)
    ...
    def build(self, bgr_input):
        ...
        rgb = tf.concat(axis=3, values=[
            (red - self.vgg_mean[0])/self.vgg_std[0],
            (green - self.vgg_mean[1])/self.vgg_std[1],
            (blue - self.vgg_mean[2])/self.vgg_std[2],
        ])
        ...
        self.pool2 = self.avg_pool(self.res8, 'pool2')
        self.logits = self.fc_layer(self.pool2, 'fc')
        self.preds = tf.nn.softmax(self.logits)
        self.top_k, self.top_k_indices = tf.math.top_k(tf.squeeze(self.preds), 5)

        self.data_dict = None

    def avg_pool(self, bottom, name):
        return tf.nn.avg_pool(bottom,
            ksize=[1, 7, 7, 1], strides=[1, 7, 7, 1], padding='SAME', name=name)
    ...
    def fc_layer(self, bottom, name):
        with tf.variable_scope(name):
            ...
            fc = tf.nn.bias_add(tf.matmul(x, tf.transpose(weights)), biases)
